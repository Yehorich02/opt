...
void print_subtree_bracketed(const tree<T>& t, typename tree<T>::iterator iRoot, 
                                      std::ostream& str=std::cout, int numSpace = 0);
...
template<class T>
void print_subtree_bracketed(const tree<T>& t, typename tree<T>::iterator iRoot, std::ostream& str, int numSpace)
	{
	if(t.empty()) return;
	if (t.number_of_children(iRoot) == 0) {
		...
		for (iChildren = t.begin(iRoot), siblingNum = 0; iChildren != t.end(iRoot); ++iChildren, ++siblingNum) {
            ...
            print_subtree_bracketed(t,iChildren,str,numSpace + 4);
